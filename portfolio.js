...
card.innerHTML='<span class="card-label">Portfolio balances</span><h2 style="font-size:18px;margin:8px 0">Managed account</h2><div id="managed-balances" class="value" style="font-size:24px">Loading…</div><p class="hint">Calculated only from approved, immutable ledger entries.</p><div style="border-top:1px solid var(--line);margin:20px 0;padding-top:18px"><h2 style="font-size:18px;margin:0 0 8px">ANXB product balance</h2><div id="anxb-product-balance" class="value" style="font-size:24px">Loading…</div><p id="anxb-product-history" class="hint">Approved airdrops and gifts can be spent on courses and upcoming NEXA products.</p></div><div style="border-top:1px solid var(--line);margin:20px 0;padding-top:18px"><h2 style="font-size:18px;margin:0 0 8px">Connected wallet</h2><div id="verified-wallets" class="hint">Loading…</div><div id="wallet-native-balance" class="value" style="font-size:22px;margin:12px 0">—</div><button class="button secondary" id="refresh-wallet-balance">Show native balance</button><p class="hint" style="margin-top:10px">Shows only the selected network’s native coin. Tokens, NFTs, and positions require a reviewed chain-data provider and are not added to the managed-account ledger.</p></div>';
activity.parentNode.insertBefore(card,activity);

const {data:{user}}=await supabase.auth.getUser();
if(user){
  const {data:anxb,error:anxbError}=await supabase.rpc('customer_anxb_commerce_summary');
  const anxbBalance=document.querySelector('#anxb-product-balance'),anxbHistory=document.querySelector('#anxb-product-history');
  if(anxbError){anxbBalance.textContent='ANXB access unavailable';anxbHistory.textContent='The ANXB product ledger has not been enabled for this environment.'}else{anxbBalance.textContent=`${Number(anxb.available_anxb||0).toLocaleString()} ANXB`;const spent=Number(anxb.spent_anxb||0),credited=Number(anxb.credited_anxb||0);anxbHistory.textContent=`${credited.toLocaleString()} credited from approved claims and gifts · ${spent.toLocaleString()} spent on NEXA products`}
